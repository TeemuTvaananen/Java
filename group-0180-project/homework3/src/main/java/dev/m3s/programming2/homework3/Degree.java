...
public class Degree{
    ...
    private final List<StudentCourse> myCourses = new ArrayList<>(MAX_COURSES);
    ...
    //own method
    public int getType(StudentCourse course){
        return course.getCourse().getCourseType();
    }
    public int getCount() {
        int sum = 0;
        for(StudentCourse course: myCourses){
    ...
    public List<Double> getGPA(int type) {
        double count = 0.0, sum = 0.0, average;
        ...
        if (type == 2) {
            ...
            for (StudentCourse course : myCourses) {
                if  (course != null) {
                    if (course.getCourse().isNumericGrade()&& getType(course) == type) {
                        count++;
                        sum += course.getGradeNum();
                    }
                }
